...
public class SetterFactory {
    ...
    public static HystrixCommand.Setter create(String interfaceName, String methodName) {

        String key = String.format("%s.%s", interfaceName, methodName);
        if (setterMap.containsKey(key)) {
            return setterMap.get(key);
        } else {
            setterMap.putIfAbsent(key, doCreate(interfaceName, methodName));
            return setterMap.get(key);
        }
    }

    private static HystrixCommand.Setter doCreate(String interfaceName, String methodName) {
